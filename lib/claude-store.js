...
function readSession(projectPath, sessionId, cap = 500) {
  ...
  for (const rec of parseLines(text)) {
    ...
    if (rec.type === 'summary' && rec.summary) {
      ...
      // Text blocks are the model's own words — kept whole like live events
      // (see truncateEvent in the claude-code adapter); only tool noise is capped.
      const content = Array.isArray(rec.message.content)
        ? rec.message.content.map((b) => (b && b.type === 'text' ? b : truncateDeep(b)))
        : truncateDeep(rec.message.content);
      events.push({ type: 'assistant', message: { content }, ts });
    }
  }
  return {
