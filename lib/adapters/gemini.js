...
class GeminiAdapter extends BaseAdapter {
  ...
  send(text) {
    ...
    // Fleet vault (spawn-time grant): MCP config file (--mcp-config here
    // takes a path, unlike Claude Code's inline JSON) and fleet context
    ...
    if (vault) args.push('--mcp-config', vault.configFile());
  ...
  // Side-channel model ask (M23): one fresh-context `gemini -p` call, no
  // session continuation, JSON parsed exactly like `send` does.
  async oneShot(prompt) {
    const settings = { model: '', ...this.ctx.getSettings() };
    const args = ['-p', prompt, '--output-format', 'json'];
    if (settings.model) args.push('-m', settings.model);
    const r = await this.quietSpawn('gemini', args, { timeoutMs: 60000 });
    if (!r || r.code !== 0) return null;
    try {
      const idx = r.stdout.indexOf('{');
      const data = JSON.parse(idx >= 0 ? r.stdout.slice(idx) : r.stdout);
      if (data && typeof data.response === 'string' && data.response.trim()) return data.response;
      return null;
    } catch {
      return null;
    }
