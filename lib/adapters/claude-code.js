...
// `claude --effort` levels. The blank first option defers to whatever the
// CLI would do anyway — its own default (high), or an effort pinned in the
// agent's env (spark's CLAUDE_CODE_EFFORT_LEVEL=medium, whose gateway
// rejects the global level). An explicit pick passes --effort, which
// overrides both.
const EFFORT_LEVELS = ['low', 'medium', 'high', 'xhigh', 'max'];

// An agent fronting another provider (spark's LiteLLM gateway, any proxy)
// pins the real model ids in env: ANTHROPIC_MODEL and the alias-mapping
// vars. When discovery at register time came back empty, offering the
// Anthropic aliases above would mislead — the gateway 404s them — so the
// pinned ids become the dropdown's fallback instead.
function pinnedModels(config) {
  ...
  for (const key of ['ANTHROPIC_MODEL', 'ANTHROPIC_SMALL_FAST_MODEL',
    ...
    if (value && !seen.has(value)) { seen.add(value); out.push({ value, label: value }); }
  }
  return out;
}
...
// The environment every `claude` spawn runs with: nested-session markers
// stripped (so the spawned CLI behaves like a fresh one), then the agent's
// own env layered on (alternate provider URL, token, model aliases) so the
// same binary can front a different backend.
function claudeEnv(config) {
  const env = { ...process.env };
  for (const key of Object.keys(env)) {
    if (key === 'CLAUDECODE' || key.startsWith('CLAUDE_CODE_')) delete env[key];
  }
  Object.assign(env, resolveEnv(config.env));
  return env;
}
...
// The model's own words — assistant text blocks and the `result` summary — are
// the payload the operator reads back, so they pass through whole even past
// MAX_STRING; truncating them mid-sentence loses information tool noise never
// does. Everything else in those events (tool_use inputs etc.) stays capped.
    ...
    return {
      ...
      message: {
        ...
        content: event.message.content.map((block) =>
          block && block.type === 'text' ? block : truncateDeep(block)),
      },
    };
  }
  if (event.type === 'result' && typeof event.result === 'string') {
    const { result, ...rest } = event;
    return { ...truncateDeep(rest), result };
  }
  return truncateDeep(event);
}
...
class ClaudeCodeAdapter extends BaseAdapter {
  ...
  settingsSchema() {
    const stored = Array.isArray(this.config.models) ? this.config.models : [];
    const pinned = pinnedModels(this.config);
    const fallback = stored.length ? stored : (pinned.length ? pinned : DEFAULT_MODELS);
    return [
      {
        key: 'model',
        label: 'Model',
      ...
      },
      {
        ...
        type: 'select',
        options: [
          { value: '', label: 'Default (CLI setting)' },
          ...EFFORT_LEVELS.map((l) => ({ value: l, label: l[0].toUpperCase() + l.slice(1) })),
        ],
        default: '',
      },
      {
  ...
  refreshAvailability() {
    ...
    return new Promise((resolve) => {
      execFile('claude', ['--version'], { timeout: 15000 }, (err) => {
        // The probe can land after a run started (spawn + send race the
        // execFile callback) — never stomp a working state with a probe result.
        if (this.isBusy()) return resolve();
  ...
  send(text) {
    ...
    const settings = { model: '', effort: '', permissionMode: 'acceptEdits', ...this.ctx.getSettings() };
    ...
    // Fleet vault (spawn-time grant): fleet context appended to the system
    // prompt, vault MCP tools via inline --mcp-config JSON (no temp files).
    const vault = this.ctx.getVaultSpawn ? this.ctx.getVaultSpawn() : null;
    if (vault) {
      args.push('--append-system-prompt', vault.preamble);
      args.push('--mcp-config', JSON.stringify({ mcpServers: { vault: vault.mcp } }));
    }
    ...
    // stdin detached: the prompt arrives via -p, and a piped-but-silent
    // stdin makes the CLI wait 3s ("no stdin data received") every run.
    const proc = spawn('claude', args, { cwd: this.ctx.getWorkspaceDir(), env, stdio: ['ignore', 'pipe', 'pipe'] });
    this.proc = proc;
    ...
    proc.stdout.on('data', (chunk) => {
      ...
      while ((idx = buf.indexOf('\n')) >= 0) {
        ...
        this.handleEvent(truncateEvent(event));
      }
    });
  ...
  // Side-channel model ask (M23): one fresh-context headless call — no
  // --resume, no vault grant, tools denied (headless `default` mode
  // auto-denies anything unapproved), single JSON result instead of a
  // stream. Used for the composer's suggested-next-step prompt.
  async oneShot(prompt) {
    const settings = { model: '', ...this.ctx.getSettings() };
    ...
    const r = await this.quietSpawn('claude', args, { env, timeoutMs: 45000 });
    if (!r || r.code !== 0) return null;
    try {
      const idx = r.stdout.indexOf('{');
      const data = JSON.parse(idx >= 0 ? r.stdout.slice(idx) : r.stdout);
      if (data && typeof data.result === 'string' && data.result.trim()) return data.result;
      return null;
    } catch {
