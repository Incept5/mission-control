...
//   { plan: 'GLM Coding Plan', monthly: 18 }  flat subscription → marginal $0
//   { perMillion: { input, output, cacheRead, cacheWrite } }  USD per 1M tokens
// The CLI's own figure is kept as `reported_cost_usd` for reference.
function applyPricing(event, pricing) {
  ...
  if (pricing.perMillion) {
    ...
    event.total_cost_usd =
      ((u.input_tokens || 0) * (p.input || 0) +
        (u.output_tokens || 0) * (p.output || 0) +
        (u.cache_read_input_tokens || 0) * (p.cacheRead ?? p.input ?? 0) +
        (u.cache_creation_input_tokens || 0) * (p.cacheWrite ?? p.input ?? 0)) / 1e6;
...
class ClaudeCodeAdapter extends BaseAdapter {
  ...
  handleEvent(event) {
    ...
    if (event.type === 'result') {
      applyPricing(event, this.config.pricing);
      this.totals.runs += 1;
