...
class OpencodeAdapter extends BaseAdapter {
  ...
  // Side-channel model ask (M23): one fresh-context `opencode run` — no `-c`,
  // so the session chain is untouched; plain-text stdout.
  async oneShot(prompt) {
    const settings = { model: '', ...this.ctx.getSettings() };
    const args = ['run'];
    if (settings.model) args.push('-m', settings.model);
    args.push(prompt);
    const r = await this.quietSpawn('opencode', args, { timeoutMs: 60000 });
