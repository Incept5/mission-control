...
class CodexAdapter extends BaseAdapter {
  ...
  send(text) {
    ...
    // Fleet vault (spawn-time grant): MCP server via `-c` TOML overrides
    // (JSON strings are valid TOML basic strings), fleet context prepended to
    // the prompt — codex has no system-prompt-append flag.
    const vault = this.ctx.getVaultSpawn ? this.ctx.getVaultSpawn() : null;
    let prompt = text;
    if (vault) {
      const s = (v) => JSON.stringify(String(v));
      args.push(
        '-c', `mcp_servers.vault.command=${s(vault.mcp.command)}`,
        '-c', `mcp_servers.vault.args=[${vault.mcp.args.map(s).join(', ')}]`,
        '-c', `mcp_servers.vault.env={ ${Object.entries(vault.mcp.env).map(([k, v]) => `${k} = ${s(v)}`).join(', ')} }`
      );
      prompt = `${vault.preamble}\n\n---\n\n${text}`;
    }
    args.push(prompt);
  ...
  // Side-channel model ask (M23): one fresh-context `codex exec` with no
  // thread resume and a read-only sandbox, plain-text stdout.
  async oneShot(prompt) {
    const settings = { model: '', ...this.ctx.getSettings() };
    const args = ['exec', '--skip-git-repo-check', '--sandbox', 'read-only'];
    if (settings.model) args.push('-m', settings.model);
    args.push(prompt);
    const r = await this.quietSpawn('codex', args, { timeoutMs: 60000 });
