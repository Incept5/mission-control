...
// to date-stamp the rate card so stale rates get re-checked) and
// `discover: 'ollama'` (the form fetches the model list live from the
// server via GET /api/ollama-models instead of trusting a hardcoded one —
// Ollama only accepts full name:tag values, so a bare guess 404s).
//
