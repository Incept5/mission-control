...
class Vault {
  ...
  // Minimal first page for a registered project (path/summary/status); M13
  // owns the refresh and retirement lifecycle. No-op when a page exists.
  async seedCatalog(project) {
    const rel = `_catalog/${project.id}.md`;
    if (fs.existsSync(this.absOf(rel))) return false;
    await this.write(rel, {
      content: [
        '---',
        'type: catalog',
        `project: ${project.id}`,
        `path: ${fmScalar(project.path)}`,
        `summary: ${fmScalar(project.summary || project.name)}`,
        'stack: []',
        'status: active',
        '---',
        '',
        `# ${project.name}`,
        '',
        String(project.summary || project.name),
        '',
      ].join('\n'),
    });
    return true;
