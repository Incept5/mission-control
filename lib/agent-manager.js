...
/* ── Suggested next steps (M22) ──────────────────────────────────────
 * When a run finishes, the composer offers its next prompt as ghost text
 * (greyed out; Tab fills it in, Enter sends). The suggestion comes from the
 * run itself — no extra model call: the model's own words when it closed
 * with a "next steps" list, else a diagnostic prompt for a failed run, else
 * a git-aware generic follow-up.
 */

// A line that opens a next-steps section. Matches the shapes models actually
// produce: `## Next steps`, `**Suggested next steps:**`, `Next steps:`,
// `Follow-ups`, `1. Next steps` — optionally with the first step inline.
const NEXT_STEPS_RE = /^\s{0,3}(?:#{1,6}\s+|\*\*+|\d+[.)]\s+)?\**\s*(?:suggested\s+|possible\s+|recommended\s+)?(?:next\s+steps?|next\s+prompts?|follow[\s-]?ups?|what(?:'s| is)\s+next)\s*\**\s*:?\s*(.*)$/i;
...
// Pull an explicit next-steps list out of closing text: the heading line
// starts the section, following list items are the steps. Prose before the
// first item is skipped (lead-ins like "Here's what I'd do:"); a fresh
// heading or any prose after the list ends it. Inline first step
// ("Next steps: run the tests") counts as one.
function extractNextSteps(text, max = 3) {
  ...
  for (const line of lines) {
    const m = inSection ? null : NEXT_STEPS_RE.exec(line);
    if (m) {
      inSection = true;
      const inline = cleanStep(m[1]);
      if (inline.length >= 5) steps.push(inline);
      continue;
    }
    if (!inSection) continue;
    if (/^\s*#{1,6}\s/.test(line)) break;   // a new heading ends the section
    if (/^\s*(?:---+|===+)\s*$/.test(line)) continue;   // rule under the heading
    const item = /^\s*(?:[-*+•]|\d+[.)])\s+/.test(line);
    if (!item) {
      if (steps.length) break;            // prose after the list ends the section
      continue;                           // blank line or lead-in prose before the first item
    }
    const step = cleanStep(line);
    if (step.length >= 5) steps.push(step);
    if (steps.length >= max) break;
  }
  return steps.slice(0, max).map((s) => s.slice(0, 240));
}

// A one-shot suggestion reply as the model tends to emit it: maybe fenced,
// maybe quoted, maybe multi-line. '' means "nothing usable" — including the
// model's own NONE verdict, which is a decision, not a failure.
function normalizeSuggestion(text) {
  let s = String(text || '').trim();
  s = s.replace(/^```[\w-]*\s*/, '').replace(/\s*```$/, '');
  s = s.replace(/^["'“‘]+/, '').replace(/["'”’]+$/, '');
  s = s.replace(/\s*\n+\s*/g, ' ').trim();
  if (/^none[.!]?$/.test(s.toLowerCase())) return '';
...
// Mask values under secret-looking keys, fail-closed — one exemption: the
// spark preset's CLAUDE_CODE_MAX_CONTEXT_TOKENS just has "TOKENS" in its
// name; it's a context-window number the form has to show and edit.
const SECRET_KEY = /^(?!CLAUDE_CODE_MAX_CONTEXT_TOKENS$).*(token|secret|key|password|passwd|credential).*$/i;
...
class AgentManager {
  ...
  // Types for the register form, each with the provider presets that can
  // pre-fill it (lib/presets.js). Types without presets get an empty list.
  agentTypes() {
  ...
  // The ~/.config/<slug>/token convention the seeded GLM agent uses
  // (~/.config/zai/token): the register form writes a pasted API key there
  // so the token only ever lives on disk — the registry stores the path,
  // resolveEnv reads the file at spawn time. Slug-anchored, so no path
  // traversal past ~/.config.
  writeTokenFile({ dir, value, overwrite } = {}) {
    const slug = String(dir || '').trim().toLowerCase();
    if (!/^[a-z0-9][a-z0-9-]{0,31}$/.test(slug)) {
      throw httpError(400, 'Directory must be a short name like "deepseek" (letters, digits, dashes)');
    }
    const secret = typeof value === 'string' ? value.trim() : '';
    if (!secret) throw httpError(400, 'Paste the API key to store');
    if (secret.includes('\n')) throw httpError(400, 'The key should be one line');
    const configDir = path.join(os.homedir(), '.config', slug);
    const file = path.join(configDir, 'token');
    const existed = fs.existsSync(file);
    if (existed && !overwrite) {
      const err = httpError(409, `${file} already exists`);
      err.exists = true;
      throw err;
    }
    fs.mkdirSync(configDir, { recursive: true, mode: 0o700 });
    fs.writeFileSync(file, secret + '\n', { mode: 0o600 });
    return { file: `~/.config/${slug}/token`, created: !existed };
  }
  ...
  statusOf(entry) {
    ...
    status.suggest = entry.suggest || null;
    return status;
  }
  ...
  pushEvent(iid, event) {
    ...
    if (this.notifier && stamped.type === 'result') {
      ...
      this.notifier.runError(entry.name, stamped.text, entry.id);
    }
    ...
    // Composer suggestions (M22): a new prompt in flight makes any old
    // suggestion stale; a finished run seeds a fresh one — unless the queue
    // already holds the operator's actual next step, which outranks a guess.
    if (stamped.type === 'user_prompt') {
      entry.suggest = null;
      entry.suggestToken = {};   // any in-flight suggestion probe is stale
    } else if (stamped.type === 'result' && !entry.queue.length) {
      this.suggestNextSteps(entry, stamped).catch(() => {});
    }
  }

  // The model's closing words for a run: its last assistant text plus the
  // result summary, where a volunteered "next steps" list would live.
  closingTextFor(entry, event) {
    const parts = [];
    const events = this.historyFor(entry.projectId);
    for (let i = events.length - 1; i >= 0; i--) {
      const ev = events[i];
      if (ev.cid !== entry.conversationId || ev.type !== 'assistant') continue;
      const text = (ev.message?.content || [])
        .filter((b) => b && b.type === 'text')
        .map((b) => b.text)
        .join('\n');
      if (text.trim()) { parts.push(text); break; }
    }
    if (typeof event.result === 'string' && event.result.trim()) parts.push(event.result);
    return parts.join('\n\n');
  }

  // Derives the composer's suggested next prompts for a finished run and
  // publishes them on the instance status. Fire-and-forget; a run starting
  // while an async step below is in flight supersedes the result via
  // suggestToken. Note the result event lands while the CLI process is still
  // closing — adapter busy-ness here means "run finished", not "stale".
  //
  // Order: extracted list → failure diagnostic → one-shot model ask (the
  // contextual path — M23; agents close with prose, not "Next steps"
  // headings, so extraction alone covered ~1 in 20 real runs) → deterministic
  // git-aware fallback only when the model call itself failed.
  async suggestNextSteps(entry, event) {
    const token = {};
    entry.suggestToken = token;
    let items = extractNextSteps(this.closingTextFor(entry, event));
    if (!items.length && failedResult(event)) {
      items = ['That run failed — reread its error output above, identify the root cause, and fix it.'];
    }
    if (!items.length) {
      let modelText = null;
      try { modelText = await entry.adapter.oneShot(this.suggestionPrompt(entry, event)); } catch {}
      if (entry.suggestToken !== token) return;
      if (modelText !== null) {
        const suggestion = normalizeSuggestion(modelText);
        // The model was consulted and had nothing (its NONE, or unusable
        // text): no ghost. A canned prompt would contradict its verdict.
    ...
    if (!items.length) {
      try {
        const st = await gitLib.status(this.getWorkspaceDir(entry));
        if (st.isRepo && st.changes && st.changes.length) {
          items = ['Review the uncommitted changes from that run, run the project\'s tests, and fix anything that fails.'];
        }
      } catch {}
      if (entry.suggestToken !== token) return;   // superseded mid-probe
    }
    if (!items.length) {
      items = ['Draft a commit message summarising that run\'s work, then list anything you left unfinished.'];
    }
    if (entry.suggestToken !== token || entry.queue.length) return;
    entry.suggest = { items, at: Date.now(), cid: entry.conversationId };
    this.broadcastStatus(entry.id);
  }

  // The operator's last ask for the finished run — the question the closing
  // text answers.
  lastPromptFor(entry) {
    const events = this.historyFor(entry.projectId);
    for (let i = events.length - 1; i >= 0; i--) {
      const ev = events[i];
      if (ev.cid !== entry.conversationId || ev.type !== 'user_prompt') continue;
      return String(ev.text || '').slice(0, 400);
    }
    return '';
  }

  // Ask handed to the adapter's one-shot call: enough of the exchange to know
  // what was requested and what was left open, strict output contract.
  suggestionPrompt(entry, event) {
    return [
      'You pick the next prompt for a coding-agent dashboard composer.',
      '',
      'The operator asked:',
      this.lastPromptFor(entry) || '(not recorded)',
      '',
      "The agent's final message:",
      this.closingTextFor(entry, event).slice(-2500) || '(empty)',
      '',
      'Write ONE short next prompt (imperative, max 25 words) the operator could send to continue this work, based strictly on what the final message says is unfinished or worth doing. Output only the prompt text — no quotes, no numbering, no explanation. If genuinely nothing remains, output exactly: NONE',
    ].join('\n');
  }
