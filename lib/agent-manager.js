...
class AgentManager {
  ...
  // Initialize the vault on boot and make sure every registered project has a
  // catalog page so the spawn preamble can list the fleet. M13 owns the
  // refresh/stamp/retire lifecycle from here on.
  async ensureVault() {
    if (!this.vaultEnabled()) return;
    try {
      await this.vault.ensure();
      for (const p of this.projects) {
        await this.vault.seedCatalog({ id: p.id, name: p.name, path: p.path, summary: p.description });
      }
    } catch (err) {
