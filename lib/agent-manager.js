...
class AgentManager {
  ...
  // Launch several agents at once, each in its own workspace/project. For
  // every row we claim an agent — preferring an idle one already pointed at
  // the target project (session continuity), then any idle one of the type
  // (repointed; per-project session pointers make that reversible), and only
  // then spawning a fresh dynamic agent. Agents already running are never
  // claimed, so every row starts concurrently instead of joining a queue.
  launchFleet(rows) {
    if (!Array.isArray(rows) || !rows.length) throw httpError(400, 'At least one launch row is required');
    if (rows.length > 12) throw httpError(400, 'At most 12 agents can be launched at once');
    const claimed = new Set();
    const results = [];
    for (const raw of rows) {
      const text = String(raw.prompt || '').trim();
      if (!text) throw httpError(400, 'Every launch row needs a prompt');
      const type = String(raw.type || 'claude-code');
      if (!adapters[type]) throw httpError(400, `Unknown agent type "${type}"`);
      const projectId = raw.projectId || null;
      const project = projectId ? this.findProject(projectId) : null;
      if (projectId && !project) throw httpError(404, `Unknown project "${projectId}"`);

      // Claim the best idle candidate: already on this project, then idle with
      // no project, then idle pointed elsewhere. Busy or queued agents are
      // skipped (their next run is spoken for); offline ones can't run now.
      const candidates = [...this.agents.values()].filter((e) =>
        e.config.type === type && !claimed.has(e.config.id) &&
        e.adapter.state === 'online' && !e.adapter.isBusy() && !e.queue.length);
      const pick =
        candidates.find((e) => (e.projectId || null) === projectId) ||
        candidates.find((e) => e.projectId === null) ||
        candidates[0] || null;
      ...
      if (pick) {
        entry = pick;
        claimed.add(entry.config.id);
        if ((entry.projectId || null) !== projectId) {
          this.setAgentProject(entry.config.id, projectId);
          mode = 'repointed';
        } else {
          mode = 'reused';
        }
        ...
        const name = String(raw.name || '').trim() ||
          (project ? `${project.name} agent` : `Fleet ${adapters[type].displayName || type} agent`);
        // addAgent registers the entry, persists it, and broadcasts it.
        const created = this.addAgent({ name, type });
        entry = this.agents.get(created.id);
        mode = 'spawned';
        if (projectId) this.setAgentProject(created.id, projectId);
      }
      if (!entry) throw httpError(500, 'Failed to create an agent for a launch row');

      this.startRun(entry.config.id, entry, text, null, this.makeOrigin('fleet'));
      results.push({
  ...
  // Swimlane data for the fleet timeline: per agent, the runs overlapping the
  // window (paired user_prompt → result events) plus the in-flight run. Prompt
  // snippets are copied at pairing time, matching the activity feed's 110-char
  // convention.
  timeline(windowMs = 6 * 3600e3) {
    const now = Date.now();
    const since = now - Math.min(Math.max(+windowMs || 0, 10 * 60e3), 7 * 864e5);
    const lanes = [];
    for (const entry of this.agents.values()) {
      const runs = [];
      let pending = null;
      for (const ev of entry.history) {
        if (ev.type === 'user_prompt') {
          pending = { start: ev.ts, prompt: String(ev.text || '').slice(0, 110), origin: ev.origin || null };
        } else if (pending && (ev.type === 'result' || ev.type === 'error')) {
          const failed = ev.type === 'error' || ev.is_error || (ev.subtype && ev.subtype !== 'success');
          if (ev.ts >= since) {
            runs.push({
              start: pending.start,
              end: ev.ts,
              durationMs: ev.duration_ms || (ev.ts - pending.start),
              failed,
              cost: ev.total_cost_usd ?? null,
              estimated: ev.estimated_cost_usd ?? null,
              pid: ev.pid || null,
      ...
      // A run still in flight has no result event yet — take it from the live
      // run record so the bar grows and can be compared against its estimate.
      if (entry.run && entry.adapter.isBusy()) {
        runs.push({
          start: entry.run.startedAt,
          end: null,
          running: true,
          estimateMs: entry.run.estimateMs,
          estimateBasis: entry.run.estimateBasis,
          durationMs: now - entry.run.startedAt,
          failed: false,
          cost: null,
          estimated: null,
          pid: entry.projectId || null,
          prompt: String(entry.adapter.currentTask || '').slice(0, 110),
          origin: entry.run.origin || null,
        });
      }
      const project = entry.projectId ? this.findProject(entry.projectId) : null;
      const lastEnd = runs.reduce((m, r) => Math.max(m, r.end || now), 0);
      lanes.push({
        id: entry.config.id,
        ...
        queue: entry.queue.length,
        runs,
        lastEnd,
      });
    }
    // Busiest lanes first: anything running, then recent activity, then idle.
    lanes.sort((a, b) =>
      Number(b.runs.some((r) => r.running)) - Number(a.runs.some((r) => r.running)) ||
      b.lastEnd - a.lastEnd);
    return { now, since, lanes };
  }
